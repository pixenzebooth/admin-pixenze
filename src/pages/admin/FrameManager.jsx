...
const FrameManager = () => {
    ...
    return (
        <div className="relative">
            ...
            <div className="fixed bottom-6 right-6 md:right-8 z-50 flex gap-2">
                {/* Moving toggle to bottom right to avoid sidebar conflict or top? Bottom right is good. Or maybe just simple tabs.
                    Let's keep the existing floating toggle but make sure it doesn't overlap sidebar on mobile. 
                    Actually, let's make it a normal tab bar above content.
                 */}
            </div>

            <div className="flex flex-col md:flex-row justify-between items-center mb-6">
                <h1 className="text-3xl font-titan text-game-accent">DASHBOARD</h1>

                <div className="bg-black/40 p-1 rounded-xl border-2 border-white/20 flex">
                    <button
                        onClick={() => setViewMode('frames')}
                        className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition ${viewMode === 'frames' ? 'bg-game-primary text-white shadow-sm' : 'text-gray-400 hover:text-white'}`}
                    >
                        <ImageIcon size={16} /> FRAMES
                    </button>
                    <button
                        onClick={() => setViewMode('winners')}
                        className={`px-4 py-2 rounded-lg text-sm font-bold flex items-center gap-2 transition ${viewMode === 'winners' ? 'bg-yellow-500 text-black shadow-sm' : 'text-gray-400 hover:text-white'}`}
                    >
                        <Users size={16} /> WINNERS
                    </button>
                </div>
            </div>
