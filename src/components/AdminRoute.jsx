...
const AdminRoute = ({ children }) => {
    ...
    if (!ADMIN_EMAILS.includes(user.email)) {
        return <div className="h-screen bg-[#02124d] flex flex-col items-center justify-center text-white p-8">
            ...
            <p className="mb-8 text-center max-w-md">You do not have clearance for this terminal. Please sign in with an administrator account.</p>
            <button onClick={() => window.location.href = '/login'} className="bg-game-accent text-black px-6 py-3 rounded-xl font-bold hover:brightness-110">
                SWITCH ACCOUNT
            </button>
        </div>;
    }
